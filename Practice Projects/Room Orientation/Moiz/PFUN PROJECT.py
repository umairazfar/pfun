...
if len(D) == 0 and len(h)!= 0 and len(T) != 0:
    ...
    for i in a:
        ...
        while b < len(i)-1:
            if len(T) == 0:
                break
            elif i[b] == 'h' and i[b+1] == '_':
                i[b+1] = T.pop(0)
                
            b = b + 1
            if b == len(i)-1 and i[b] == 'h' and i[b-1] == '_':
                i[b-1] = T.pop(0)
                break
          
elif len(D)!=0 and len(h)!= 0 and len(T)!= 0:
    ...
    if '_' in i and len(h)!=0 :
        ...
        while b < len(i)-1:
            if len(h) == 0:
                ...
                pass
            b = b + 1
            if b == len(i) - 1 and i[b-1] != 'h' and len(h) != 0 and i[b] == '_':
                i[b] = h.pop(0)
                break
...
file1.write('There are ' + str(num) + ' empty space(s)' + '\n')
file1.write('\n')
file1.write(str2)
file1.write('\n')
if len(h)!=0 :
    file1.write(str(len(h)) + ' seat(s) cannot be arranged\n') 
if len(T)!=0 :
    file1.write(str(len(T)) + ' table(s) cannot be arranged\n')
if len(D)!=0 :
    file1.write(str(len(D)) + ' computer(s) cannot be arranged\n')
file1.close()    
